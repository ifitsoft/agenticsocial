...
def test_the_renderer_has_no_day_path(mjs):
    ...
    assert "--day" not in mjs
    assert "content/" not in mjs


def test_the_renderer_refuses_to_run_without_a_plan(mjs):
    ...
    assert "--plan" in mjs
    assert "agsoc video render" in mjs


def test_the_renderer_has_no_timing_arithmetic_of_its_own(mjs):
    ...
    assert "plan.total_frames" in mjs
    assert "plan.fps" in mjs
    for arithmetic in ("* FPS", "*FPS", "const FPS", "Math.round(total"):
        assert arithmetic not in mjs, arithmetic


def test_a_plan_with_no_frame_count_is_refused_before_a_browser_is_launched(mjs):
    ...
    assert guard in mjs
    assert mjs.index(guard) < mjs.index(launch), (
...
def test_the_two_hand_written_episodes_are_still_present(mjs):
