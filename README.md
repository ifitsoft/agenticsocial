...
Note: approval is per-status, not per-content — if you (or an agent) edit a
variant after approving it, set `status: in_review` in its frontmatter, then
run `agsoc review` and `agsoc approve` again before posting.
